...
async function save() {
  ...
  try {
    ...
    if (props.country) {
      ...
      country = await countriesService.update(props.country.id, {
        name_en: form.name_en,
        name_ar: form.name_ar,
        code: form.code,
      })
      if (form.is_active !== props.country.is_active) {
        country = form.is_active
          ? await countriesService.activate(props.country.id)
          : await countriesService.deactivate(props.country.id)
      }
    } else {
    ...
    initial = JSON.stringify(form)
    app.pushToast('success', isEdit.value ? t('countries.updated') : t('countries.created'))
    emit('saved', country)
  } catch (e) {
...
<template>
  <form class="space-y-5" novalidate @submit.prevent="save">
    <div class="grid gap-4 sm:grid-cols-2">
      <FormField for-id="c-name-en" :label="t('countries.nameEn')" :error="fieldErrors.name_en" required>
        <input id="c-name-en" v-model="form.name_en" class="input" autocomplete="off" required>
      </FormField>
      <FormField for-id="c-name-ar" :label="t('countries.nameAr')" :error="fieldErrors.name_ar" required>
        <input id="c-name-ar" v-model="form.name_ar" class="input" dir="rtl" autocomplete="off" required>
      </FormField>
    </div>

    <FormField for-id="c-code" :label="t('countries.code')" :error="fieldErrors.code" :hint="t('countries.codeHint')" required>
      <input id="c-code" v-model="form.code" class="input uppercase" autocomplete="off" maxlength="8" required>
    </FormField>
    ...
    <div class="flex items-center justify-end gap-2 border-t border-border pt-4">
      ...
      <button type="submit" class="btn btn-primary min-w-28" :disabled="saving">
        ...
        {{ saving ? t('common.saving') : (isEdit ? t('common.save') : t('countries.createAction')) }}
      </button>
    </div>
  </form>
</template>
